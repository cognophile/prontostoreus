            ...
            $this->execute(
                ...
                BEGIN
                    IF (NEW.company_id IS NULL AND NEW.customer_id IS NULL) THEN
                        ...
                        SET MESSAGE_TEXT = 'INSERT: An address must be related to a company OR a customer';
                    END IF;
                    IF (NEW.company_id IS NOT NULL AND NEW.customer_id IS NOT NULL) THEN
                        SIGNAL SQLSTATE '45000'
                        SET MESSAGE_TEXT = 'INSERT: An address cannot be linked to a company AND a customer';
                    END IF;
                END"
            );
        }
